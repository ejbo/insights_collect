"""xAI Grok — X / Twitter live search 强项."""
...
_PRICE = {"grok-4": (3.0, 15.0), "grok-3": (2.0, 10.0)}
...
class GrokProvider(SearchProvider):
    """xAI exposes an OpenAI-compatible REST API at api.x.ai."""
    name = "grok"
    default_search_model = "grok-4"
    default_reasoning_model = "grok-4"
    ...
    async def search(
        ...
        prompt = (
            f"Search for recent expert viewpoints about: {query}\n"
            f"Time window: {time_window.start.date()} to {time_window.end.date()}.\n"
            f"Prioritize statements from notable people on X (Twitter), interviews, blog posts.\n"
            f"{'用中文回答' if lang == 'zh' else 'Answer in English.'}"
        )
        ...
        snippets: list[RawSnippet] = [
            RawSnippet(snippet=content[:400], url=u, source_domain=_domain(u),
